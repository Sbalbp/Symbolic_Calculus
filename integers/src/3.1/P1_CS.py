...
if sys.version_info[0] < 3:
	print("\nERROR: Este programa debe ejecutarse con una version de Python igual o superior a la 3.\n")
	sys.exit()		
...
entrada = None
while (entrada == None):
	entrada = input('Seleccione una opcion: ').upper()
	
	if (entrada == 'Q' or i == 5):
		exit()
	elif   int(entrada) > 0 and int(entrada) <= 7:
		entrada = int(entrada)
	else:
		entrada = None
		i += 1
...
if(entrada < 7):
	...
	#Leemos los numeros
	ruta = input('Introduzca la ruta del fichero en el que se encuentra el primer entero\n(dejar en blanco para introducir por teclado)\n')
	if(len(ruta) > 0):
		entero1.leeNumero(ruta)
	else:
		entero1.leeNumero()
		
	ruta = input('Introduzca la ruta del fichero en el que se encuentra el segundo entero\n(dejar en blanco para introducir por teclado)\n')
	if(len(ruta) > 0):
